package th1;

public class GennericStackClient {

    private static void stackOfStrings() {
        MyGenericStack<String> stack = new MyGenericStack();
        stack.push("five");
        stack.push("four");
        stack.push("three");
        stack.push("two");
        stack.push("one");
        System.out.println("1.1 size stack after push operaion : " + stack.size());
        System.out.println("1.2 Pop elemet from stack");
        while (!stack.isEmpty()) {
            System.out.printf(" %s",stack.pop());

        }
        System.out.println("\n3.3 size of stack after pop operation" + stack.size());
    }

    private static void stackOftegers() {
        MyGenericStack<Integer> stack = new MyGenericStack<>();
        stack.push(5);
        stack.push(4);
        stack.push(3);
        stack.push(2);
        stack.push(1);
        System.out.println("2.1 Size of stack after push operation" + stack.size());
        System.out.println("2.2 pop elements from stack");
        while (!stack.isEmpty()) {
            System.out.printf(" %d", stack.pop());
        }
        System.out.println("\n3.3 Size of stack affter pop opration" + stack.size());
    }

    public static void main(String[] args) {
        System.out.println("1. stack of integers");
        stackOftegers();
        System.out.println("\n2. stack of string");
        stackOfStrings();
    }

}
